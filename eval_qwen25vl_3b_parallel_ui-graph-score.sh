...
python -m evaluation.ss_v2_eval \
    --save_path "${SAVE_PATH}/drop_${EVAL_VISUAL_REDUCT_RATIOS}_${SCORER_TYPE}_${MODEL_TYPE}_parallel" \
...
python -m evaluation.ss_v2_eval \
    --save_path "${SAVE_PATH}/drop_${EVAL_VISUAL_REDUCT_RATIOS}_${SCORER_TYPE}_${MODEL_TYPE}_sequential" \
    --visual_reduct_ratio "${EVAL_VISUAL_REDUCT_RATIOS}" \
    --scorer_type "${SCORER_TYPE}" \
    --model_type "${MODEL_TYPE}" \
    --data_path "${UI_GROUNDING_BENCH_BASE_DIR}/ScreenSpot-V2" \
