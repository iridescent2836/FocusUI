...
SAVE_PATH="eval_results/focusui_3b/screenspot_pro_new_scorer"
...
for drop in $EVAL_VISUAL_REDUCT_RATIOS; do
    for scorer in $SCORER_TYPE; do
        echo "ScreenSpot-Pro | drop=${drop} | scorer=${scorer} | combined scorer"
...
for drop in $EVAL_VISUAL_REDUCT_RATIOS; do
    echo "ScreenSpot-Pro | drop=${drop} | scorer=scorer | new scorer only"
    python -m evaluation.ss_pro_eval \
